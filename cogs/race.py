...
class Race:
    ...
    async def _start_race(self, ctx):
        ...
        wait = settings['Time']
        await self.bot.say(":triangular_flag_on_post: 動物賽跑開始！輸入 {}race "
                           "enter 加入比賽 :triangular_flag_on_post:\n{}比賽將會在"
                           " {} 秒後開始\n\n**{}** 加入了"
                           "比賽".format(ctx.prefix, ' ' * 25, wait, author.mention))
        await asyncio.sleep(wait)
        await self.bot.say(":checkered_flag: 比賽開始！ :checkered_flag:")
        ...
        embed.add_field(name='-' * 99, value='{} 贏了!'.format(data['Winner']))
        embed.title = "比賽結束，第一名者可以輸入 =claim領取獎勵"
    ...
    async def _claim_race(self, ctx):
        ...
        if data['Winner'] != author:
            return await self.bot.say("你尚未贏得任何比賽")
        try:
