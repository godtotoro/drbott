...
class Economy:
    ...
    async def balance(self, ctx, user: discord.Member=None):
        ...
        if not user:
            ...
            try:
                await self.bot.say("{} 你的現金{}$".format(
                    user.mention, self.bank.get_balance(user)))
            except NoAccount:
                await self.bot.say("{} 你尚未擁有"
                                   " 貓貓銀行帳戶，輸入`{}bank register`"
                                   "進行創建".format(user.mention,
                                                          ctx.prefix))
        else:
            try:
                await self.bot.say("{}的現金是：{}$".format(
        ...
        if self.bank.account_exists(author):
            if id in self.payday_register[server.id]:
                ...
                if seconds >= self.settings[server.id]["PAYDAY_TIME"]:
                    ...
                    await self.bot.say(
                        "{} 簽到冷卻中：{}.".format(author.mention, dtime))
            else:
            ...
            await self.bot.say("{} 你沒有銀行帳戶，輸入"
                               "`{}bank register`來創建".format(
    ...
    async def slot(self, ctx, bid: int):
        ...
        try:
            ...
            await self.bot.say("{}你沒有銀行帳戶，"
                               "輸入`{}bank register`創建".format(author.mention, ctx.prefix))
        except InsufficientBalance:
            await self.bot.say("{} You need an account with enough funds to"
                              "play the slot machine.".format(author.mention))
        except OnCooldown:
            await self.bot.say("拉霸機冷卻中，請稍等{}秒".format(slot_time))
        except InvalidBid:
            await self.bot.say("Bid must be between {} and {}".format(settings["SLOT_MIN"],settings["SLOT_MAX"]))
