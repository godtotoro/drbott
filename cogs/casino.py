...
class Casino(CasinoBank):
    ...
    async def _join_casino(self, ctx):
        ...
        try:
            ...
            return await self.bot.say(_("{} 以經擁有帳戶").format(user.name))
        else:
            name = settings["System Config"]["Casino Name"]
            await self.bot.say(_("註冊成功!歡迎來到{} !\n新人獎勵"
                                 "500籌碼已發送"
                                 "\n祝你遊玩愉快喵!").format(name))
    ...
    async def _transfer_casino(self, ctx, user: discord.Member, chips: int):
        ...
        if not super().membership_exists(author):
            return await self.bot.say("{} 尚未擁有賭場帳戶".format(author.name))

        if not super().membership_exists(user):
            return await self.bot.say("{} 尚未擁有賭場帳戶".format(user.name))
        ...
        if not cooldown:
            try:
                ...
                return await self.bot.say(_("你不能轉送給自己"))
            except BotNotAUser:
                return await self.bot.say(_("你不能轉送給機器人"))
            except InsufficientChips:
                return await self.bot.say(_("籌碼不足無法轉送"))
            else:
                logger.info("{}({}) transferred {} {} to {}({}).".format(author.name, author.id,
                                                                         chip_name, chips,
                                                                         user.name, user.id))
            await self.bot.say(_("{} 轉送 {} {} 籌碼給 {}.").format(author.name, chip_name,
                                                                       chips, user.name))
        else:
    ...
    async def _exchange_casino(self, ctx, currency: str, amount: int):
        ...
        # Logic for choosing Credits
            ...
            if amount <= 0 and amount % chip_multiple != 0:
                ...
                await self.bot.say(_("轉換{}現金至 {} {} 籌碼\n助你在"
                                     "{}賭場玩的愉快喵！").format(amount, chip_amount, chip_name,
                                                                  casino_name))
            else:
                await self.bot.say(_("現金不足"))
    ...
    @commands.command(name="payday", pass_context=True)
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def payday(self, ctx):
        ...
        if not super().membership_exists(user):
            await self.bot.say("你尚未加入{}輸入`{}casino join來加入"
                               "".format(casino_name, ctx.prefix))
        else:
            ...
            if not cooldown:
                if settings["Players"][user.id]["Membership"]:
                    ...
                    msg = _("這是你的簽到獎勵{} {} 籌碼").format(amount, chip_name)
                else:
                    payday = settings["System Config"]["Default Payday"]
                    super().deposit_chips(user, payday)
                    msg = _("這是你的簽到獎勵{} {} 籌碼").format(payday, chip_name)
            else:
                msg = cooldown
            await self.bot.say(msg)

    @commands.command(name="b", pass_context=True)
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def b(self, ctx):
        ...
        try:
            ...
            await self.bot.say(_("你尚未加入{}輸入{}casino join來加入").format(casino_name, ctx.prefix))
        else:
            await self.bot.say(_("```Python\n你擁有 {} {} 籌碼```").format(balance, chip_name))
    ...
    async def hilo(self, ctx, choice: str, bet: int):
        ...
        await self.bot.say(_("骰子擲出並滾了幾圈..."))
        ...
        if choice in outcome:
            ...
            if self.threshold_check(settings, amount):
                ...
                super().deposit_chips(user, amount)
                msg += _("```Python\n你贏得了{} {} 籌碼```").format(amount, chip_name)
        else:
            msg += _("抱歉，你猜錯了。總和是 {} ({}).").format(result, outcome[0])
    ...
    async def cups(self, ctx, cup: int, bet: int):
        ...
        if check:
            ...
            await self.bot.say(_("莊家快速的移動杯子..."))
            ...
            if cup == outcome:
                ...
                msg = _("恭喜，你猜中了! 硬幣在第 {}杯下!").format(outcome)
                ...
                msg = _("抱歉，你猜錯了。硬幣在第 {}杯下.").format(outcome)
            # Save the results of the game
            super().save_system()
    ...
    async def coin(self, ctx, choice: str, bet: int):
        ...
        if check:
            ...
            await self.bot.say(_("硬幣擲出在天空翻了幾圈..."))
            await asyncio.sleep(2)

            # Begin game logic to determine a win or loss
            if choice == outcome:
                amount = int(round(bet * settings["Games"]["Coin"]["Multiplier"]))
                msg = _("恭喜，你猜中了! 硬幣是{}!").format(outcome)
                ...
                msg = _("抱歉，你猜錯了。硬幣是{}.").format(outcome)
            # Save the results of the game
            super().save_system()
        # Send a message telling the user the outcome of this command
        await self.bot.say(msg)
    ...
    async def dice(self, ctx, bet: int):
        ...
        if check:
            ...
            await self.bot.say(_("骰子扔出並轉了幾圈"
                                 "..."))
            die_one = random.randint(1, 6)
            die_two = random.randint(1, 6)
    ...
    async def allin(self, ctx, multiplier: int):
        ...
        if check:
            ...
            await self.bot.say(_("你把你所有籌碼放入機器中並拉下拉桿..."))
            await asyncio.sleep(3)
            ...
            if outcome == 0:
                super().deposit_chips(user, amount)
                msg = _("```Python\n 數字{}，Jackpot!! 你贏得了 {} {} "
                        "chips!!```").format(outcome,amount, chip_name)
                settings["Players"][user.id]["Won"]["Allin"] += 1
            else:
                msg = (_("數字 {} ，抱歉你花光了你所有的"
                         "{}，什麼都沒拿到").format(outcome,chip_name))
            # Save the results of the game
            super().save_system()
        # Send a message telling the user the outcome of this command
        await self.bot.say(msg)
    ...
    async def _balance_setcasino(self, ctx, user: discord.Member, chips: int):
        ...
        try:
            ...
            await self.bot.say(_("```Python\n使用職權之力將 {} 籌碼設定為 "
                                 "{} {} 籌碼```").format(user.name, chips, chip_name))
    ...
    async def _chipname_setcasino(self, ctx, *, name: str):
        ...
        msg = ("Changed the name of your chips to {0}.\nTest Display:\n"
               "```Python\nCongratulations, 你贏得了 50 {0} 籌碼```".format(name))
    ...
    async def war_game(self, user, settings, deck, amount):
        ...
        await self.bot.say(_("莊家抽出兩張牌"
                             "各放在一邊..."))
        await asyncio.sleep(2)
        await self.bot.say(_("**咻!**"))
        ...
        if pc > dc:
            ...
            await self.bot.say(_("你跟莊家的牌相同 **{}**!\n這就代表說... "
                                 "**賭場戰爭開始**! 你可以選擇投降(Surrender/Ffs)並取回下注金額的一半 "
                                 "或進入進入賭場戰爭(War)\n你必須加倍你的籌碼 "
                                 "如果點數較大，則只會贏取與額外加注相等的金額"
                                 "點數再次相同，則可贏取加倍籌碼 **請輸入英文決定**").format(player_card))
            ...
            if choice is None or choice.content.title() in (_("Surrender"), _("Ffs")):
                ...
                await self.bot.say(_("莊家削掉三張牌並在抽出兩張牌 "
                                     "各放在一邊..."))
                await asyncio.sleep(3)
                await self.bot.say(_("**咻!**"))
                ...
                await self.bot.say(_("你投降了，你回收了一半的下注籌碼"))
                outcome = "Surrender"
                amount = int(amount / 2)
    ...
    async def blackjack_game(self, dh, user, amount, deck):
        ...
        if choice is None or choice.content.title() == _("Stay"):
            ...
            try:
                ...
                await self.bot.say(_("籌碼不足，請選擇 hit 或 stay."))
                ...
                if choice2 is None or choice2.content.title() == _("Stay"):
                    ...
                    while count < 21:
                        ...
                        if count >= 21:
                            break
                        msg = (_("{}\n你的卡牌: {}\n你的分數: {}\n莊家顯示的牌: "
                                 "{}\nHit 或 stay?").format(user.mention, ", ".join(ph), count,
                                                            dh[0]))
                        await self.bot.say(msg)
        ...
        # Continue game logic in a loop until the player's count is 21 or bust.
            ...
            while count < 21:
                ...
                if count >= 21:
                    break
                msg = (_("{}\n你的卡牌: {}\n你的分數: {}\n莊家顯示的牌: "
                         "{}\nHit 或 stay?").format(user.mention, ", ".join(ph), count, dh[0]))
                await self.bot.say(msg)
    ...
    def minmax_check(bet, game, settings):
        ...
        if mi <= bet <= mx:
            ...
            if mi != mx:
                msg = (_("錯誤，下注金過小或過大，最低下注金額是 {} 最高下注金額為 "
                         "{} ").format(mi, mx))
            else:
    ...
    def war_results(self, settings, user, outcome, player_card, dealer_card, amount):
        ...
        if outcome == "Win":
            ...
            if self.threshold_check(settings, amount):
                ...
                super().deposit_chips(user, amount)
                msg += (_("**\*\*\*\*\*\*你贏了!\*\*\*\*\*\***\n```Python\n你贏得了 {} {} "
                          "籌碼```").format(amount, chip_name))

        elif outcome == "Loss":
            msg += _("======莊家贏了======")
        else:
            super().deposit_chips(user, amount)
            msg = (_("======**{}**======\n:flag_white: 我投降 :flag_white:\n=================="
                     "\n{} {} 這是你的下注籌碼.").format(user.name, amount, chip_name))
    ...
    def blackjack_results(self, settings, user, amount, ph, dh):
        ...
        if dc > 21 >= pc or dc < pc <= 21:
            ...
            if self.threshold_check(settings, total):
                ...
                msg += (_("**\*\*\*\*\*\*你贏了!\*\*\*\*\*\***\n```Python\nYou just "
                          "won {} {} 籌碼```").format(total, chip_name))
                super().deposit_chips(user, total)
        elif pc > 21:
            msg += _("======爆炸!======")
        elif dc == pc <= 21:
            msg += (_("======超過21點======\n你的 {} {} 籌碼已收回 "
                      "").format(amount, chip_name))
            amount = int(round(amount))
            super().deposit_chips(user, amount)
        elif pc < dc <= 21:
            msg += _("======莊家贏了======").format(user.name)
        # Save results and return appropriate outcome message.
        super().save_system()
        return msg
    ...
    def game_checks(self, settings, prefix, user, bet, game, choice, choices):
        ...
        try:
            ...
            msg = (_("你尚未註冊賭博帳戶 請輸入`{}casino join "
                     "`.").format(casino_name, prefix))
            return msg
        except InsufficientChips:
            msg = _("你的籌碼不足")
            return msg
