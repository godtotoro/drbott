...
class Shop:
    ...
    async def shop_table_split(self, user, data):
        headers = ["物品", "販售數量", "販售價格", "折扣"]
        groups = [data[i:i + 12] for i in range(0, len(data), 12)]
        pages = len(groups)
        if pages == 1:
            page = 0
            table = tabulate(groups[page], headers=headers, stralign="left", numalign="left")
            msg = ("```ini\n{}``````Python\n頁數 {} "
                   "總共販售 {} 像物品```".format(table, pages, len(data)))
            return msg
        else:
            ...
            if response is None:
                ...
                page = 0
            try:
                table = tabulate(groups[page], headers=headers, stralign="left",
                                 numalign="left")
                msg = ("```ini\n{}``````Python\nYou are viewing page {} of {}. "
    ...
    def bordered(text):
        ...
        width = max(len(s) + 5 for s in lines)
        res = ["+" + "-" * width + '+']
        for s in lines:
    ...
    def table_builder(self, settings, column1, column2, column3, column4, shop_name):
        header = "```"
        header += self.bordered(shop_name + " 販售物品")
        header += "```"
